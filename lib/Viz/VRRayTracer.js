...
export default class RayTracer extends Renderer {
  ...
  resizeCanvas() {
    ...
    /* The above code is a JavaScript comment. It is defining a constant variable `sizeFactor` with a
    value of 0.05. However, the code is commented out, so it will not be executed when the program
    runs. */
    const sizeFactor = 0.05
    // const sizeFactor = 1
    // NOTE: if yours is too slow, chante the target height here, e.g. 512
    const tgtHeight = height; 
    // const tgtHewwight = 512; 
    ...
    console.log(this._offScreenTextureLeft, this._offScreenTextureRight)
    if (this._tracer) {
      ...
      this._tracer.createBindGroup.bind(this._offScreenTextureLeft, this._offScreenTextureRight);
    }
    this._bindGroups = [
  ...
  onFrame(time) {
    ...
    renderPass.setBindGroup(0, this._bindGroups[this._idx]);
    ...
    for (const source of session.inputSources) {
      ...
      if (gamepad) {
        for (const button in gamepad.buttons) {
          if (button.pressed) {
            const event = new CustomEvent('vrEvent', { detail: { button: button } });
