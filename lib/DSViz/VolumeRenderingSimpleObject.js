...
export default class VolumeRenderingSimpleObject extends RayTracingObject {
  ...
  resetParticles() {
    ...
    for (let i = 0; i < this._numParticles; ++i) {
      ...
      // y position
      this._particles[vars * i + 1] = Math.random(); //(Math.random() * 0.5); // [-1, 1] 
      // z position
  ...
  async getRaycastChecks() {
    ...
    encoder.copyBufferToBuffer(this._cameraBuffers[(this._step + 0) % 2], 0, this._stageBuffer, 0, (Math.ceil(this._cameras[0]._pose.byteLength / 16) * 16 +
                                                                                              Math.ceil(this._cameras[0]._focal.byteLength / 16) * 16 +
                                                                                              Math.ceil(this._cameras[0]._resolutions.byteLength / 16) * 16) * this._cameras.length);
