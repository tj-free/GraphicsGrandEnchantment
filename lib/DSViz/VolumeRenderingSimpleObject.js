...
export default class VolumeRenderingSimpleObject extends RayTracingObject {
  ...
  async createGeometry() {
    ...
    for (let i = 0; i < this._cameras.length; i++) {
      this._device.queue.writeBuffer(this._cameraBuffer, offset, this._cameras[i]._pose);
      this._device.queue.writeBuffer(this._cameraBuffer, this._cameras[i]._pose.byteLength + offset, this._cameras[i]._focal);
      this._device.queue.writeBuffer(this._cameraBuffer, this._cameras[i]._pose.byteLength + this._cameras[i]._focal.byteLength + offset, this._cameras[i]._resolutions);
  ...
  updateGeometry() {
    ...
    for (let i = 0; i < this._cameras.length; i++) {
      ...
      this._device.queue.writeBuffer(this._cameraBuffer, offset, this._cameras[i]._resolutions);
      offset += this._cameras[i]._pose.byteLength + this._cameras[i]._focal.byteLength + this._cameras[i]._resolutions.byteLength;
    }
  }
  ...
  updateCameraFocal() {
    ...
    for (let i = 0; i < this._cameras.length; i++) {
      this._device.queue.writeBuffer(this._cameraBuffer, offset, this._cameras[i]._focal);
      offset += this._cameras[i]._pose.byteLength + this._cameras[i]._focal.byteLength + this._cameras[i]._resolutions.byteLength;
    }
  }
  ...
  compute(pass) {
    ...
    pass.dispatchWorkgroups(Math.ceil(this._wgWidth / 16), Math.ceil(this._wgHeight / 16), 2); // dispatch
  }
}
